...
export async function GET(request: NextRequest) {
    ...
    const API_KEY = 'AIzaSyA0TME5f5g7ViZ7WpxI1R2qru8rh3_TPZg'; // User provided key

    try {
        let videos = [];

        if (playlistId) {
            // Fetch from Playlist via YouTube Data API
            ...
            if (data.items) {
                videos = data.items.map((item: any) => ({
                    ...
                    type: 'sermon' // Explicitly set type for main page filtering
                }));
            }
        } else {
            // Fallback to RSS Feed (Channel Uploads)
            ...
            // entry can be an array or single object if only one video exists
